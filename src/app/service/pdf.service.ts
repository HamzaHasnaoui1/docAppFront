...
export class PdfService {
  ...
  async generateOrdonnancePdf(ordonnance: Ordonnance): Promise<void> {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text(`Dr. ${medecin.nom}`, 15, 40);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(`Spécialité: ${medecin.specialite}`, 15, 45);
    doc.text(`Email: ${medecin.email}`, 15, 50);
    doc.text(`Téléphone: ${medecin.numeroTelephone}`, 15, 55);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(`${patient.titre || ''} ${patient.nom}`, 50, 75);
    doc.text(`Date de naissance: ${this.formatDate(patient.dateNaissance)}`, 50, 80);
    ...
    doc.text(`Dr. ${medecin.nom}`, 130, yPos + 20);
